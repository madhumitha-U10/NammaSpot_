...
const ID_KEYS: Record<(typeof SHEET_TABLES)[number], string> = { sellers: "sellerId", products: "productId", categories: "categoryId", customers: "customerId", enquiries: "enquiryId", reviews: "reviewId" };
...
export const fetchSheetBundle = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ rows: Record<string, SheetRow[]>; error?: string }> => { const { readTables } = await import("@/lib/sheets-cache.server"); return readTables(data.tables); });

async function postToSheets(payload: Record<string, unknown>) {
  const base = process.env["SHEETS_API_BASE"] ?? SHEETS_API_BASE_FALLBACK;
  const res = await fetch(base, { method: "POST", redirect: "follow", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify(payload) });
...
export const authenticateAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => (await postToSheets({ action: "authenticateAdmin", password: data.password })).data as { token: string; expiresAt: number });

export const verifyAdmin = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => z.object({ token: z.string().min(1) }).parse(data))
  .handler(async ({ data }) => Boolean(((await postToSheets({ action: "verifyAdmin", token: data.token })).data as { valid?: boolean } | null)?.valid));
...
export const updateSheetRow = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => z.object({ table: tableSchema, id: z.string().min(1), row: rowSchema, adminToken: z.string().optional() }).parse(data))
  .handler(async ({ data }) => {
    const idKey = ID_KEYS[data.table];
    return postToSheets({ action: "update", table: data.table, data: { ...data.row, [idKey]: data.id }, adminToken: data.adminToken });
  });
