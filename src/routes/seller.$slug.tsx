...
function SellerProfile() {
  ...
  const { data: reviews } = useStoreData(() => { const s = sellerBySlug(slug); return s ? reviewsBySeller(s.id) : []; });
    ...
    <SiteShell>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 lg:px-6">
        <Tabs defaultValue="catalogue"><TabsList><TabsTrigger value="catalogue">Catalogue</TabsTrigger><TabsTrigger value="about">About</TabsTrigger><TabsTrigger value="reviews">Reviews ({reviews?.length ?? 0})</TabsTrigger></TabsList>
          ...
          <TabsContent value="reviews" className="mt-6 max-w-2xl space-y-3">{(reviews ?? []).length === 0 && <p className="text-sm text-muted-foreground">No reviews published yet.</p>}{(reviews ?? []).map((r) => <div key={r.id} className="card-soft p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">{r.customerName}</p><Rating value={r.rating} /></div><p className="mt-2 text-sm text-muted-foreground">{r.comment}</p><p className="mt-2 text-xs text-muted-foreground">{r.createdAt}</p></div>)}</TabsContent>
        </Tabs>
