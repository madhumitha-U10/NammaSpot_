 ...
 * Script properties required: SHEET_ID and ADMIN_PASSWORD. WRITE_TOKEN is optional.
 */
...
function ensureHeaders_(sh, keys) {
  ...
  var missing = [];
  keys.forEach(function(k) { if (lower.indexOf(String(k).toLowerCase()) === -1) missing.push(k); });
...
function rows_(tabName) {
  ...
  return values.filter(function(r) { return r.some(function(v) { return String(v).trim().length > 0; }); }).map(function(r) {
    var obj = {};
...
function columnIndex_(head, key) {
  ...
  for (var i = 0; i < head.length; i++) if (String(head[i]).trim().toLowerCase() === lower) return i;
  return -1;
}
...
function findRow_(sh, head, idKey, id) {
  ...
  for (var i = 0; i < values.length; i++) if (String(values[i][0]).trim() === String(id).trim()) return i + 2;
  return -1;
}
...
function writeRecord_(tableKey, mode, data) {
  ...
  try {
    ...
    if (mode === "create" && rowNumber === -1) {
      var fresh = head.map(function(h) { var k = columnKey_(data, h); return k === null ? "" : data[k]; });
      sh.appendRow(fresh);
    ...
    head.forEach(function(h, i) { var k = columnKey_(data, h); if (k !== null) current[i] = data[k]; });
...
function doPost(e) {
  try {
    ...
    if (action === "adminLogin") {
      ...
      if (!expectedPassword) return fail_("ADMIN_PASSWORD script property is not configured");
      return String(body.data && body.data.password || "") === String(expectedPassword)
        ? ok_({ authenticated: true })
        : fail_("Invalid admin credentials");
    }
